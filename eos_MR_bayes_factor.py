...
def mass_radius_bfactor(N):
    
    # Obtains bayes factor of each target eos using mass/radius/kappa files
    ...
    t_kappa_dir = "mass_radius_files/target_eos/"

    r_kappa_dir = "mass_radius_files/refer_eos/"

    t_dir = "mass_lambda_files/kappa_method/target_eos/"

    r_dir = "mass_lambda_files/kappa_method/refer_eos/"
    ...
    for eos in target_eos:
        ...
        m, r, k = modsel.getMR(eos, m_min=min_mass, N=1000)
 
        output = np.vstack((m, r, k)).T
        outputfile = t_kappa_dir+eos+'.txt'
        np.savetxt(outputfile, output, fmt="%f\t%f\t%f")

        s, masses, _, max_mass = modsel.getEoSInterpFromMRFile(t_kappa_dir+eos+'.txt')
        Lambdas = s(masses)
    ...
    min_mass = modsel.getMinMass(refer_eos) # Small function I added to the GWXtreme script that returns an eos's min_mass
    m, r, k = modsel.getMR(refer_eos, m_min=min_mass, N=1000)
 
    output = np.vstack((m, r, k)).T
    outputfile = r_kappa_dir+refer_eos+'.txt'
    np.savetxt(outputfile, output, fmt="%f\t%f\t%f")

    s, masses, _, max_mass = modsel.getEoSInterpFromMRFile(r_kappa_dir+refer_eos+'.txt')
    Lambdas = s(masses)
    lambdas = (Lambdas / lal.G_SI) * ((masses * lal.MRSUN_SI) ** 5)
    ...
    with open("results/bayes_factors_MR_SLY.json", "w") as f:
