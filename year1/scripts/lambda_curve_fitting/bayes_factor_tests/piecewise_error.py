...
def get_piecewise_error():
    ...
    # Will get statistical error of the piecewise polytrope method's bayes 
    # factor results compared to those from lal models
    ...
    for eos in tar_list:
        ...
        error.append((abs(lal_bayes_factor-bayes_factor)/lal_bayes_factor)*100)
    
    outputfile = "results/piecewise_error_values.txt"
    np.savetxt(outputfile, error, fmt="%f")

    # Now lets plot the eos associated with the lowest and highest bayes factor

    accurate_eos = tar_list[np.argmin(error)]
    accurate_pw_eos = t_eos_vals[accurate_eos]

    modsel.plot_func([accurate_eos, accurate_pw_eos], filename="accurate_eos_plot.pdf")

    inaccurate_eos = tar_list[np.argmax(error)]
    inaccurate_pw_eos = t_eos_vals[inaccurate_eos]

    modsel.plot_func([inaccurate_eos, inaccurate_pw_eos], filename="inaccurate_eos_plot.pdf")
