...
def multipleEventBFs(Trials=1000):
    ...
    output = "data/BNS/BFs/16simulations_2D_3D_BFs_10000trial.json"
...
def multipleEventPlots():
    ...
    for uncert_index in range(2):
        ...
        for label in labels:
            ...
            for eos in eosList:
                BFs.append(data[label][eos][0])
                uncert = data[label][eos][uncert_index] # default
                if type(uncert) == list: uncert = np.std(uncert) # unless double standard deviation
                uncerts.append(uncert)
        ...
        plt.savefig("plots/BNS/BFs/16simulations_2D_3D_BFs_10000trial_{}.png".format(uncert_label),bbox_inches="tight")
